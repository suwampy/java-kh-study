...
if (!nexacro.FileDownload) {
	...
	_pFileDownload.on_create_contents = function () {
		...
		nexacro._create_filedownload_handle(this._on_download_onload, this);
	};

	_pFileDownload.on_destroy_contents = function () {
		nexacro._IconText.prototype.on_destroy_contents.call(this);

		nexacro._destroy_filedownload_handle(this);
	};
	...
	_pFileDownload.download = function (url, targetpath) {
		...
		if (url != undefined) {
			...
			ret = nexacro._download(url, this._hidden_frame_handle, initname, targetpath);
		}
		else if (downloadurl && downloadurl != undefined && downloadurl != "") {
			downloadurl = nexacro._getServiceLocation(downloadurl);
			ret = nexacro._download(downloadurl, this._hidden_frame_handle, initname, targetpath);
		}
